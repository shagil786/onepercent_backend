...
router.post('/addTask', async(req, res) => {
    ...
    const startDateFormatted = toDate(startDate), endDateFormatted = toDate(endDate);
...
router.get('/allTask', async(req, res) => {
    try {
        ...
        const sortDirection = req?.query?.sortDirection ? req?.query?.sortDirection : 'asc';
        let tasks;
        switch(filter) {
            case 'due':
            ...
            case 'today':
                ...
                tasks = await Task.find({
                    $and: [{startDate: {$gte: todayStart}}, {endDate: {$lte: todayEnd}}]
            ...
            case 'tommorrow':
                ...
                tasks = await Task.find({
                    $and: [{startDate: {$gte: tomorrowStart}}, {endDate: {$lte: tomorrowEnd}}]
                }).sort({startDate: orderMap[sortDirection]});
                break;
            case 'priority':
                        ...
                        $addFields: {
                            priorityOrder: {
                                $switch: {
                                    ...
                                    default: 0 // default case (if priority is not low, medium, or high)
                                }
                            }
                        }
                    },
                    {
                        $sort: { priorityOrder: orderMap[sortDirection] } // 1 for ascending, -1 for descending
                    },
                    {
                        $project: { priorityOrder: 0 } // Remove the temporary field if you don't need it in the output
                    }
                ]);
                break;
            case 'creation-date':
                tasks = await Task.find().sort({createdDate: orderMap[sortDirection]});
                break;
            default:
                tasks = await Task.aggregate([
                    {
...
router.get('/tasks/count', async(req, res) => {
    try {
        ...
        const result = [{name: "pending", value: 0}, {name: "completed", value: 0}, {name: "in-progress", value: 0}];
...
router.put('/tasks/:taskId', async(req, res) => {
    try {
        ...
        res.status(200).json({ success: true, data: existingTask, message: "Task updated successfully" });
    } catch (errpr) {
